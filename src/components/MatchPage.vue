          ...
          <tr>
            <th>
              <div class="comment-title">
                User Comments on this Match <br />
              </div>
            </th>
          </tr>
...
export default {
  ...
  mounted() {
    ...
    fetch(
      "https://tfb308.herokuapp.com/api/v1/match/" + this.matchId,
      requestOptions
    )
  ...
  methods: {
    refereeVote() {
      ...
      const requestOptions = {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: this.returnObject.referee,
          rate: this.rating,
        }),
      };
    ...
    comment() {
      ...
      fetch("https://tfb308.herokuapp.com/api/v1/comment", requestOptions)
        .then((response) => response.json())
        .then((data) => {
          if (data.status === "200") {
